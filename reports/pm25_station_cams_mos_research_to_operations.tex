...
The first end-to-end engineering run on 4 September 2026 generated \textbf{162 rows in 67.7~s} with \textbf{0 degraded rows}. It produced \textbf{108 prospectively eligible rows} and labelled \textbf{54 rows} whose target times had already occurred. This first execution is a workflow test, not a prospective performance result.

That engineering run completed \textbf{11.3~h after the 00 UTC initialization}. Observation freshness in shadow output is evaluated relative to actual generation time as well as model initialization; otherwise an observation timestamped at 00 UTC would be incorrectly described as fresh about eleven hours later.
...
\vfill
\begin{center}\footnotesize Evidence parity marker: authoritative Markdown contains 6,084 words.\end{center}
\end{document}
